...
<div class="form-group row">
    ...
    <div class="col-lg-10">
        <input name="{{$name}}" value="{{Request::old($name) ? Request::old($name) : $model->name}}" type="text" class="form-control" required placeholder="اكتب عنوان المستوي">
    </div>
</div>

@php $document = "document"; @endphp
<div class="form-group row">
    <label class="col-form-label col-lg-2">المذكرة<span class="text-danger">*</span></label>
    <div class="col-lg-10">
        <textarea required name="{{$document}}"  class="form-control summernote">{{Request::old($document) ? Request::old($document) : $model->document}}</textarea>
    </div>
</div>
